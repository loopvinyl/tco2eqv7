...
if not df_podas.empty:
    ...
    df_podas_destino = (
        df_podas.groupby(COL_DESTINO, dropna=False)["MASSA_FLOAT"]
        .sum()
        .reset_index()
    )
    ...
    st.dataframe(
        df_view[[COL_DESTINO, "Massa (t)", "Percentual (%)"]],
    ...
    if massa_aterro_t > 0:
        ...
        st.caption(
            "Cálculo baseado no IPCC 2006. "
            "Emissões de CH₄ da compostagem consideradas desprezíveis."
        )
    else:
