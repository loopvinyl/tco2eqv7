...
def criar_graficos_simulacao(massa_anual, cenario):
    ...
    if fracoes['Redução Percentual'] > 0:
        ...
        unidades = ['t CO₂eq', 'R$/ano', 'R$/ano', 'R$/ano']
        
        bars = ax4.bar(categorias, valores, color=['#2ecc71', '#3498db', '#9b59b6', '#f39c12'])
...
def criar_graficos_interativos(massa_anual, cenario):
    """Cria gráficos interativos usando Plotly"""
    
    fracoes = calcular_simulacao(massa_anual, cenario)
    
    # Gráfico 1: Comparação de cenários (Plotly)
    cenarios_comparacao = ['Atual', 'Economia Circular', 'Otimizado']
    dados_comparacao = {
        'Aterro': [85, 40, 20],
        'Reciclagem': [8, 35, 45],
        'Compostagem': [7, 25, 35],
    ...
    for i, destino in enumerate(['Aterro', 'Reciclagem', 'Compostagem']):
        fig1.add_trace(go.Bar(
            name=destino,
            x=cenarios_comparacao,
            y=dados_comparacao[destino],
            marker_color=['#e74c3c', '#3498db', '#2ecc71'][i],
            text=[f"{v}%" for v in dados_comparacao[destino]],
            textposition='auto'
        ))
    
    fig1.update_layout(
        title='Comparação de Cenários - Composição da Destinação',
        barmode='stack',
    ...
    # Gráfico 2: Benefícios econômicos (Plotly)
    if fracoes['Redução Percentual'] > 0:
        beneficios = {
            'Categoria': ['Redução GEE', 'Reciclagem', 'Compostagem', 'Total'],
            'Valor (R$)': [
                fracoes['Valor_Carbono_R$'],
                fracoes['Valor_Reciclagem_R$'],
                fracoes['Valor_Compostagem_R$'],
                fracoes['Valor_Total_R$']
            ]
        }
        ...
        fig2 = px.bar(
            ...
            title='Benefícios Econômicos Anuais'
        )
    ...
    if 'Município' in colunas:
        ...
        if dados_municipio_completo is not None and len(dados_municipio_completo) > 0:
            ...
            with st.container():
                ...
                # Layout em 3 colunas para métricas principais
                ...
                # Layout para gráficos e tabelas
                col_graf1, col_graf2 = st.columns(2)
                
                with col_graf1:
                    st.markdown("##### 📊 Gráficos Estáticos (Matplotlib)")
                ...
                with col_graf2:
                    ...
                    fig1, fig2 = criar_graficos_interativos(massa_total_municipio, cenario)
